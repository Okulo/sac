...
class UpdateDebtors extends Command
{
     ...
     * @var string
     */
    protected $signature = 'cloudpayments:updatedebtors'; // Date example: 2020-12-31
    ...
    public function handle()
    {
        ...
        foreach ($debtors as $debtor){
            ...
            //ниже условие для Клауда
            if($debtor->payment_type == "cloudpayments" && isset($debtor->cp_subscription_id)){
                ...
                if(isset($card) && ($endedAt < $now)){
                    ...
                    try{
                        ...
                        $objetoRequest->request->add([
                            'subscriptionId' => $debtor->id,
                            'cardId' => $card->id
                        ]);

                        $subscr = new SubscriptionController();
                        $subscr->manualWriteOffPayment($objetoRequest);

                    } catch (\Throwable $e) {
                        throw new \Exception('Ошибка авто списания стредств с должника по  Cloudpayments. ID:  '.$debtor->id, 500);
                    }
                }
            }
            //ниже условие для Питека Pitech
